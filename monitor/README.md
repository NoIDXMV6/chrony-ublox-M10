...
### Дашборд не загружается (зависает на «Инициализация…»)
- Откройте консоль браузера (F12) – там будет выведена конкретная ошибка.
- Если ошибка `t is not a function`, проверьте, что файл локализации (`ru_locale.txt` или `en_locale.txt`) загружается корректно, и что в `monitor.js` в функции `renderSystem` не переопределена переменная `t` (температура должна называться `sysTemp`).
